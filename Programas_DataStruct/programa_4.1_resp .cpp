...
void llenar_lista(Nodo *&primero, Nodo *&ultimo){
	...
	if( (d >= 1 && d <=31) && (m >= 1 && m <= 12) && (a >= 1930 && a <= 2022)){

    nuevo_nodo->fecha.dia=d;
	nuevo_nodo->fecha.mes=m;
	nuevo_nodo->fecha.anio=a;
...
void print(Nodo *actual){
	...
	cout<<"Fecha de Nacimiento\t: "<<actual->fecha.dia<<" /"<<actual->fecha.mes<<"/"<<actual->fecha.anio<<endl;
